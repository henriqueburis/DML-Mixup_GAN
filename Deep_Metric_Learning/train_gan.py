...
novelset = MultiFolderLoader(root=args.data_dir, transform=data_transform, num_classes = args.num_classes, start_indx = 0, img_type = "."+args.im_ext, ret_class=False)#(root=args.data_dir, transform=data_transform, num_classes = args.val_classes, start_indx = args.num_classes, img_type = "."+args.im_ext, ret_class=False)
...
with torch.no_grad():
    ...
    print(generator_network(fixed_noise, fixed_features).shape[2])
    if not(generator_network(fixed_noise, fixed_features).shape[2] == 256):
